...
description: Expert knowledge base for the EPG4002 Aprendizaje No Supervisado course (Magíster en IA, PUC Chile, Prof. Jonathan Acosta). Covers unsupervised learning — similarity/dissimilarity measures, hierarchical clustering (single/complete/average/Ward), non-hierarchical clustering (K-means, K-medoids/PAM, K-modes, K-prototypes), Gower distance for mixed data, choosing the number of clusters (elbow, CH(K), Hartigan, Silhouette, Gap statistic), PCA, and the Python/R code used in class. Use for any EPG4002 question — homework, labs, controls, project, syllabus. Also trigger on clustering, agrupamiento, distancia de Gower/euclidiana/Manhattan/Minkowski/Mahalanobis, Jaccard, dendrograma, enlace simple/completo/promedio/Ward, K-medias, PAM, K-prototype, método del codo, Silhouette, Gap statistic, Calinski-Harabasz, componentes principales, PCA, valores/vectores propios, or course datasets (iris, datos mixtos de Agresti). Respond in Spanish when the user writes in Spanish.
---
