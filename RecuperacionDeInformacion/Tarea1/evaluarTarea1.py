# INF3841 - TAREA 1 - RECUPERACIÓN DE INFORMACIÓN
    ...
    def leer_archivo_gt(self, filename):
        ...
        with open(filename) as f:
            for linea in f:
                ...
                if gt.imagen_r != "-":
                    self.total_por_tipo[gt.tipo] = (
                        self.total_por_tipo.get(gt.tipo, 0) + 1
                    )
                    self.num_queries += 1
        if len(self.lista) != 2000:
            print(
                "{}: archivo GT invalido ({} queries)".format(filename, len(self.lista))
            )
        ...
        with open(filename) as f:
            for linea in f:
                ...
                try:
                    ...
                    if len(partes) != 3:
                        print(
                            "{}: Formato incorrecto (se esperan 3 columnas)".format(
                                filename
                            )
                        )
                        sys.exit(1)
                    ...
                    if det.distancia < 0:
                        print(
                            "{}: Formato incorrecto (distancia={})".format(
                                filename, det.distancia
                            )
                        )
                        sys.exit(1)
...
class Metricas:
    ...
    def get_resultado_por_tipo(self):
        ...
        for tipo in self.recall_por_tipo:
            val += "      {:12s}={:4} correctas ({:.0f}%)\n".format(
                tipo, self.correcta_por_tipo[tipo], 100 * self.recall_por_tipo[tipo]
            )
        return val
    ...
    def get_metricas1(self):
        val = "respuestas_evaluadas={}".format(
            self.ignoradas + self.duplicadas + self.correctas + self.incorrectas
        )
    ...
    def get_metricas2(self):
        return "precision={:.3f}  recall={:.3f}  F1={:.3f}".format(
...
class Evaluador:
    ...
    def imprimir_resultado_general(self, nombre_dataset):
        ...
        print(
            "  De {} casos, {} se detectaron correctamente ({:.0f}%)".format(
                self.metricas.num_queries,
    ...
    def imprimir_incorrectas(self, incorrectas_a_mostrar):
        print(
            "  Las {} respuestas incorrectas de menor distancia:".format(
                incorrectas_a_mostrar
            )
        )
        cont = 0
...
def evaluar_resultado_en_dataset(nombre_dataset, filename_gt, filename_resultados):
    ...
    print("---------")
    print(
        "Comparando {} con respuestas correctas en {}".format(
            filename_resultados, filename_gt
        )
    )
...
def validar_tiempo_maximo(t0):
    ...
    # el enunciado dice que no puede demorar mas de 10 minutos
    if segundos > 10 * 60:
        print("La tarea no puede demorar más de 10 minutos en procesar un dataset!!")
        sys.exit(1)
...
def leer_archivos(ruta):
    archivos = []
    for file in list(pathlib.Path(ruta).rglob("*")):
        archivos.append(ruta + "/" + str(file))
    archivos.sort()
    print("  {} archivos en {}".format(len(archivos), ruta))
    return archivos


def verificar_archivos(ruta, archivos_esperados):
    archivos_nuevos = leer_archivos(ruta)
    borrado = False
    for file in archivos_nuevos:
        if file not in archivos_esperados:
            if os.path.isdir(file):
                print("  BORRANDO CARPETA CREADA: " + file)
                shutil.rmtree(file)
                borrado = True
            else:
                print("  BORRANDO ARCHIVO CREADO: " + file)
                os.remove(file)
                borrado = True
    if borrado:
        print("EL PROGRAMA NO PUEDE MODIFICAR EL DATASET DE EVALUACIÓN")
        sys.exit(1)


def ejecutar_tarea(nombre_dataset, dataset_q, dataset_r, dir_evaluacion):
    ...
    filename_resultados = os.path.join(
        datos_temporales, "resultados.{}.txt".format(nombre_dataset)
    )
    ...
    if esCppWindows:
        ...
        comando1 = ["tarea1-parte1.exe", dataset_r, dir_descriptores_r]
        comando2 = ["tarea1-parte2.exe", dataset_q, dir_descriptores_r, filename_resultados]
    elif esCppLinux:
        ## comandos para binarios (C++) en Linux
        comando1 = ["./tarea1-parte1", dataset_r, dir_descriptores_r]
        comando2 = ["./tarea1-parte2", dataset_q, dir_descriptores_r, filename_resultados]
    # leer los archivos en los datasets
    archivos_q = leer_archivos(dataset_q)
    archivos_r = leer_archivos(dataset_r)
    # parte 1
    ejecutar_comando(comando1)
    # verificar que el dataset no haya sido modificado
    verificar_archivos(dataset_q, archivos_q)
    verificar_archivos(dataset_r, archivos_r)
    # parte 2
...
def evaluar_en_dataset(nombre_dataset, dir_evaluacion):
    ...
    if not os.path.isfile(filename_gt):
        ...
        sys.exit(1)
    t0 = time.time()
    filename_resultados = ejecutar_tarea(
        nombre_dataset, dataset_q, dataset_r, dir_evaluacion
    )
    validar_tiempo_maximo(t0)
...
def evaluar_tarea1(datasets):
    print("INF3841 - Recuperación de Información")
    print("Evaluación Tarea 1 2026")
    ...
    for nombre_dataset in resultados:
        ...
        f1s.append(f1)
        print(
            "    F1-score en {}: {:.3f}   (tiempo={:.1f} segundos)".format(
                nombre_dataset, f1, segundos
            )
        )
